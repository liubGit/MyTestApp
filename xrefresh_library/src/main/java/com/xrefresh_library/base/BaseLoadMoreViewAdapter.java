...
public abstract class BaseLoadMoreViewAdapter<T> extends RecyclerView.Adapter<BaseRecViewHolder> {
    ...
    private XRefreshListView loadingView;
    ...
    private int onePageNum = ConstantValue.PAGE_COUNT;//一页显示的数量
    ...
    public BaseRecViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (isFooter(viewType)) {
            ...
            changeViewSize(view);
            BaseRecViewHolder holder = new BaseRecViewHolder(view);
    ...
    public void onLoadSuccess(int count) {
        ...
        if (count < onePageNum) {
            ...
            int status = LoadingView.SUCCESS;
            if (getListSize() == 0) {
                msg = tipByNotData;
                status = LoadingView.NOT_DATA;
            }
            if (loadingView != null)
                loadingView.onLoadComplete(status, null);
            if (footerView != null) {
            ...
            if (loadingView != null)
                loadingView.onLoadComplete(LoadingView.SUCCESS, null);
            if (footerView != null) {
    ...
    public void onLoadFailure(String msg) {
        ...
        //列表还没数据且加载失败
        if (loadingView != null && getListSize() == 0)
            loadingView.onLoadComplete(LoadingView.FAILURE, msg);
        if (footerView != null) {
            progressBar.setVisibility(View.GONE);
            tipContent.setText(msg);
        }
    ...
    public void setLoadingView(XRefreshListView loadingView) {
