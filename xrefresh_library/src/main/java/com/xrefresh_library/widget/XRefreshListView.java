...
import com.xrefresh_library.callback.IHeaderCallBack;
...
public class XRefreshListView extends LinearLayout {
    // -- header view
    ...
    private void initWithContext(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                    R.styleable.XRefreshListView, 0, 0);
            try {
                isHeightMatchParent = a.getBoolean(
                        R.styleable.XRefreshListView_isHeightMatchParent, true);
                isWidthMatchParent = a.getBoolean(
                        R.styleable.XRefreshListView_isHeightMatchParent, true);
                autoRefresh = a.getBoolean(
                        R.styleable.XRefreshListView_autoRefresh, false);
                autoLoadMore = a.getBoolean(
                        R.styleable.XRefreshListView_autoLoadMore, true);
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                a.recycle();
    ...
    private void attachContentView() {
        mContentView.setContentView(XRefreshListView.this.getChildAt(1));
...
//        mFooterCallBack.onStateRefreshing();
    ...
    public void setOnXRefreshViewListener(XRefreshViewListener listener) {
        mRefreshViewListener = listener;
        mContentView.setXRefreshViewListener(listener);
    }
